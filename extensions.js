...
const waiter = setInterval(()=>{
  // ВАЖНО: `state` объявлена в index.html через `const`, поэтому она НЕ становится
  // свойством window (в отличие от `var` и function-деклараций типа openDetailModal).
  // Проверять нужно саму переменную `state`, а не `window.state` — иначе условие
  // никогда не станет истинным, и весь модуль молча зависает навсегда.
  let ready = false;
  try { ready = typeof state !== 'undefined' && state && state.data && state.data.length; } catch(e){}
  if (ready) { clearInterval(waiter); extInit().catch(e=>console.error('[ext]',e)); }
}, 200);
